<x-layouts.main>
    <h1 class="text-xl font-semibold text-gray-800 mb-4">Akun Saya</h1>
            ...
            <div class="ms-3 grow">
                <div id="vertical-tab-with-border-1" role="tabpanel" aria-labelledby="vertical-tab-with-border-item-1">
                    <div class="lg:grid grid-cols-5">
                        <form action="{{ route('user.account.update') }}" method="post" class="mt-5 col-span-2"
                            autocomplete="off">
                            @method('PUT')
                            @csrf
                            ...
                            <div class="mb-3">
                                <label for="nimInput" class="block text-sm font-medium mb-2">Nomor Induk
                                    Mahasiswa</label>
                                <input type="text" id="nimInput" name="nim"
                                    ...
                                    placeholder="Masukkan nim" value="{{ $user->nim }}" required>
                            </div>
                                ...
                                <select id="prodiSelect" name="prodi_id"
                                    ...
                                    @foreach ($daftarProdi as $prodi)
                                        <option value="{{ $prodi->id }}"
                                            @if ($prodi->id == $user->prodi_id) selected @endif>
                                            {{ $prodi->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div id="vertical-tab-with-border-2" class="hidden" role="tabpanel"
                    ...
                    <div class="lg:grid grid-cols-5">
                        <form action="{{ route('user.account.password') }}" method="post" class="mt-5 col-span-2"
                            autocomplete="off">
                            @method('PUT')
                            @csrf
